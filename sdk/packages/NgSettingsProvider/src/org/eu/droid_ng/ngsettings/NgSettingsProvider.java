...
/**
 * The NgSettingsProvider serves as a {@link ContentProvider} for Ng specific settings
 */
public class NgSettingsProvider extends ContentProvider {
    public static final String TAG = "NgSettingsProvider";
    private static final boolean LOCAL_LOGV = false;

    private static final boolean USER_CHECK_THROWS = true;

    public static final String PREF_HAS_MIGRATED_NG_SETTINGS =
            "migrated_settings_to_ng_3_0";

    private static final Bundle NULL_SETTING = Bundle.forPair("value", null);

    // Each defined user has their own settings
    protected final SparseArray<NgDatabaseHelper> mDbHelpers = new SparseArray<NgDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(NgSettings.AUTHORITY, NgDatabaseHelper.NgTableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(NgSettings.AUTHORITY, NgDatabaseHelper.NgTableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(NgSettings.AUTHORITY, NgDatabaseHelper.NgTableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(NgSettings.AUTHORITY, NgDatabaseHelper.NgTableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating NgSettingsProvider");
        ...
        mUriBuilder.authority(NgSettings.AUTHORITY);
    ...
    /**
     * Migrates Ng settings for all existing users if this has not been run before.
     */
    private void migrateNgSettingsForExistingUsersIfNeeded() {
        boolean hasMigratedNgSettings = mSharedPrefs.getBoolean(PREF_HAS_MIGRATED_NG_SETTINGS,
                false);

        if (!hasMigratedNgSettings) {
            long startTime = System.currentTimeMillis();

            for (UserInfo user : mUserManager.getUsers()) {
                migrateNgSettingsForUser(user.id);
            }

            mSharedPrefs.edit().putBoolean(PREF_HAS_MIGRATED_NG_SETTINGS, true).commit();
    ...
    /**
     * Migrates Ng settings for a specific user.
     * @param userId The id of the user to run Ng settings migration for.
     */
    private void migrateNgSettingsForUser(int userId) {
        synchronized (this) {
            if (LOCAL_LOGV) Log.d(TAG, "Ng settings will be migrated for user id: " + userId);

            // Migrate system settings
            int rowsMigrated = migrateNgSettingsForTable(userId,
                    NgDatabaseHelper.NgTableNames.TABLE_SYSTEM, NgSettings.System.LEGACY_SYSTEM_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ng system table");

            // Migrate secure settings
            rowsMigrated = migrateNgSettingsForTable(userId,
                    NgDatabaseHelper.NgTableNames.TABLE_SECURE, NgSettings.Secure.LEGACY_SECURE_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ng secure table");

            // Migrate global settings
            rowsMigrated = migrateNgSettingsForTable(userId,
                    NgDatabaseHelper.NgTableNames.TABLE_GLOBAL, NgSettings.Global.LEGACY_GLOBAL_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ng global table");
        }
    }

    /**
     * Migrates Ng settings for a specific table and user id.
     * @param userId The id of the user to run Ng settings migration for.
     * @param tableName The name of the table to run Ng settings migration on.
     * @param settings An array of keys to migrate from {@link Settings} to {@link NgSettings}
     * @return Number of rows migrated.
     */
    private int migrateNgSettingsForTable(int userId, String tableName, String[] settings) {
        ...
        for (String settingsKey : settings) {
            ...
            if (tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_SYSTEM)) {
                settingsValue = Settings.System.getStringForUser(contentResolver, settingsKey,
                        userId);
            }
            else if (tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_SECURE)) {
                settingsValue = Settings.Secure.getStringForUser(contentResolver, settingsKey,
                        userId);
            }
            else if (tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_GLOBAL)) {
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(NgSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        switch (method) {
            // Migrate methods
           case NgSettings.CALL_METHOD_MIGRATE_SETTINGS:
                migrateNgSettingsForExistingUsersIfNeeded();
                return null;
           case NgSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER:
                migrateNgSettingsForUser(callingUserId);
                return null;

            // Get methods
            case NgSettings.CALL_METHOD_GET_SYSTEM:
                return lookupSingleValue(callingUserId, NgSettings.System.CONTENT_URI,
                        request);
            case NgSettings.CALL_METHOD_GET_SECURE:
                return lookupSingleValue(callingUserId, NgSettings.Secure.CONTENT_URI,
                        request);
            case NgSettings.CALL_METHOD_GET_GLOBAL:
                return lookupSingleValue(callingUserId, NgSettings.Global.CONTENT_URI,
                        request);

            // Put methods
            case NgSettings.CALL_METHOD_PUT_SYSTEM:
                enforceWritePermission(org.eu.droid_ng.platform.Manifest.permission.WRITE_SETTINGS);
                callHelperPut(callingUserId, NgSettings.System.CONTENT_URI, request, args);
                return null;
            case NgSettings.CALL_METHOD_PUT_SECURE:
                enforceWritePermission(
                        org.eu.droid_ng.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, NgSettings.Secure.CONTENT_URI, request, args);
                return null;
            case NgSettings.CALL_METHOD_PUT_GLOBAL:
                enforceWritePermission(
                        org.eu.droid_ng.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, NgSettings.Global.CONTENT_URI, request, args);
                return null;

            // List methods
            case NgSettings.CALL_METHOD_LIST_SYSTEM:
                return callHelperList(callingUserId, NgSettings.System.CONTENT_URI);
            case NgSettings.CALL_METHOD_LIST_SECURE:
                return callHelperList(callingUserId, NgSettings.Secure.CONTENT_URI);
            case NgSettings.CALL_METHOD_LIST_GLOBAL:
                return callHelperList(callingUserId, NgSettings.Global.CONTENT_URI);

            // Delete methods
            case NgSettings.CALL_METHOD_DELETE_SYSTEM:
                enforceWritePermission(org.eu.droid_ng.platform.Manifest.permission.WRITE_SETTINGS);
                return callHelperDelete(callingUserId, NgSettings.System.CONTENT_URI,
                        request);
            case NgSettings.CALL_METHOD_DELETE_SECURE:
                enforceWritePermission(
                        org.eu.droid_ng.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, NgSettings.Secure.CONTENT_URI,
                        request);
            case NgSettings.CALL_METHOD_DELETE_GLOBAL:
                enforceWritePermission(
                        org.eu.droid_ng.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, NgSettings.Global.CONTENT_URI,
                        request);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        NgDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (NgDatabaseHelper.NgTableNames.TABLE_GLOBAL.equals(tableName)) {
            validateGlobalSettingNameValue(name, value);
        } else if (NgDatabaseHelper.NgTableNames.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (NgDatabaseHelper.NgTableNames.TABLE_SECURE.equals(tableName)) {
    ...
    private int deleteForUser(int callingUserId, Uri uri, String selection,
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            NgDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end NgSettings API, and updates that
    ...
    /**
     * Tries to get a {@link NgDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link NgDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private NgDatabaseHelper getOrEstablishDatabase(int callingUser) {
    ...
    /**
     * Check if a {@link NgDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking ng settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new ng settings db helper for user " + userId);
                }
                dbHelper = new NgDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        ...
        final boolean protectedTable =
                NgDatabaseHelper.NgTableNames.TABLE_SECURE.equals(tableName) ||
                NgDatabaseHelper.NgTableNames.TABLE_GLOBAL.equals(tableName);
        ...
        throw new SecurityException(
                String.format("Permission denial: writing to ng settings requires %1$s",
                        org.eu.droid_ng.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return NgDatabaseHelper.NgTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return NgDatabaseHelper.NgTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return NgDatabaseHelper.NgTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return NgDatabaseHelper.NgTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_SYSTEM : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link NgSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_GLOBAL);
        if (tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_SYSTEM)) {
            property = NgSettings.System.SYS_PROP_NG_SETTING_VERSION;
        } else if (tableName.equals(NgDatabaseHelper.NgTableNames.TABLE_SECURE)) {
            property = NgSettings.Secure.SYS_PROP_NG_SETTING_VERSION;
        } else if (isGlobal) {
            property = NgSettings.Global.SYS_PROP_NG_SETTING_VERSION;
        }
    ...
    private void validateGlobalSettingNameValue(String name, String value) {
        NgSettings.Validator validator = NgSettings.Global.VALIDATORS.get(name);
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        NgSettings.Validator validator = NgSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        NgSettings.Validator validator = NgSettings.Secure.VALIDATORS.get(name);
